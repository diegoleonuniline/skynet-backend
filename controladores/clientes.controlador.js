  ...
  try {
    ...
    if (tarifa > 0) {
      ...
      if (diaInstalacion < diaCorte) {
        ...
        fechaVencimientoProrrateo = new Date(anio, mes, diaCorte);
      } else if (diaInstalacion > diaCorte) {
        diasProrrateo = (30 - diaInstalacion) + diaCorte;
        mesProrrateo = mes + 1;
        if (mesProrrateo > 11) { mesProrrateo = 0; anioProrrateo++; }
        fechaVencimientoProrrateo = new Date(anioProrrateo, mesProrrateo, diaCorte);
      }
      ...
      if (diasProrrateo > 0) {
        ...
        await connection.query(
          ...
          [generarUUID(), clienteId, periodo, fecha_instalacion, 
           ...
           montoProrrateo.toFixed(2), diasProrrateo]
        );
        cargosGenerados++;
      }
    }

    // Costo de instalación (va a tabla instalaciones)
    if (parseFloat(costo_instalacion) > 0) {
      await connection.query(
        `INSERT INTO instalaciones (id, cliente_id, monto, fecha_instalacion, notas, estado, creado_por)
         VALUES (?, ?, ?, ?, 'Instalación inicial', 'pendiente', ?)`,
        [generarUUID(), clienteId, costo_instalacion, fecha_instalacion, req.usuario?.usuario_id || null]
      );
      cargosGenerados++;
    }
    ...
    if (tarifa > 0) {
      ...
      const fechaPrimeraMensualidad = new Date(anioMensualidad, mesMensualidad, diaCorte);
      ...
      await connection.query(
        ...
        [generarUUID(), clienteId, periodo,
         ...
         fechaPrimeraMensualidad.toISOString().split('T')[0], tarifa]
      );
      cargosGenerados++;
    }
...
async function registrarInstalacion(req, res) {
  ...
  try {
    ...
    if (parseFloat(costo_instalacion) > 0) {
      await connection.query(
        `INSERT INTO instalaciones (id, cliente_id, monto, fecha_instalacion, notas, estado, creado_por)
         VALUES (?, ?, ?, ?, 'Instalación', 'pendiente', ?)`,
        [generarUUID(), id, costo_instalacion, fecha_instalacion, req.usuario?.usuario_id || null]
      );
      cargosGenerados++;
    }
